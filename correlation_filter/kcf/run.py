...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if selectingObject:
            ...
            roi = np.array([ix, iy, w, h])
            boundingbox, peak = tracker.update(frame, roi)
            tracker.retrain(frame, roi)
            t1 = time()
            ...
            x, y, w, h = boundingbox[:]
            roi = np.array([x, y + h, w, h])
            loc, peak_value = tracker.detect(tracker.get_tmpl(), tracker.getFeatures(frame, roi, 0, 1.0))
