...
在实现的时候，是把每一个 cell 中的梯度幅值按照 x^2, xy, yx, y^2 的权重分配到包括 cell 在内四个 cell 中去。权重的具体分配如下所示：
a.Cell(i, j) = r * x^2
b.Cell(i + nearest[ii], j) = yx
c.Cell(i, j + nearest[jj]) = xy
d.Cell(i + nearest[ii], j + nearest[jj]) = y^2
...
def func2(r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize):
    ...
    # i, j, ii, jj 会遍历图像 image 中每一个像素点，一共 height * width 个像素点，并且把这些像素点对应的幅值 r 分配到相邻的 3 个 cell 上
        ...
        for j in range(sizeX):
            ...
            for ii in range(k):
                for jj in range(k):
                    ...
                    # r[k * i + ii, j * k + jj] 表示目标图像 (i,j) 处 cell 中某一个像素点的梯度幅值，r 矩阵的大小和图像一样大，
                    # 表示原图像中每一像素点的梯度值，r.shape = (height, width)
                    # 同理，alfa[k * i + ii, j * k + jj, m]（m = 0,1）表示 alfa 中 [0,180) 和 [0,360) 两种角度
                    if (i * k + ii > 0) and (i * k + ii < height - 1) and (j * k + jj > 0) and (j * k + jj < width - 1):

                        # 当前 (i,j) 处的 cell 所获得的的 r 梯度幅值（权重为 x * x）
                        ...
                        #  (i + nearest[ii] ,j) 处的 cell 所获得的的 r 梯度幅值（权重为 y * x），也就是 (i,j) 的正上或者正下的 cell
                        ...
                        # （i, j + nearest[jj]) 处的 cell 所获得的 r 梯度幅值（权重为 x * y），也就是（i,j）的正左或者正右的 cell
                        ...
                        # （i + nearest[ii], j + nearest[jj]) 处的 cell 所获得的 r 梯度幅值（权重为 y * y），也就是（i,j）的左上或者右下的 cell
...
# 开始进行邻域归一化
# partOfNorm 的 shape 为 [sizeY * sizeX, 1]，也就是一个向量，表示每一个 cell 前 9 个梯度幅值的平方和
...
def func3(partOfNorm, mappmap, sizeX, sizeY, p, xp, pp):
    ...
    for i in range(1, sizeY + 1):
        for j in range(1, sizeX + 1):
            # pos1 代表的是 mappmap 矩阵中的哪个 cell，一个 cell 是长为 27 的向量
            ...
            # mappmap 中前 9 个方向梯度被归一化后，保存在 newData[i,j] 的 [0,8) 向量中
            # mappmap 中后 18 个方向梯度被归一化后，保存在 newData[i,j] 的 [36,54) 向量中
...
# pp = 31, yp = 4, xp = 9, p = 108，nx = sqrt(18), ny = sqrt(4)
@jit(cache=True)
def func4(mappmap, p, sizeX, sizeY, pp, yp, xp, nx, ny):
    # mappmap 的 shape 为 [sizeX * sizeY * 108]
    ...
    for i in range(sizeY):
        for j in range(sizeX):
            ...
            for-1 表示在 [36, 108) 这个区间中每隔 18 个选一个，也就是在 [36,54),[54,72),[72,90),[90,108) 四个区间中各选一个数，累加，然后乘以 1 / sqrt(4)，
                  比如选择 [36,54,72,90] 这四个数，第二次选择 [37,55,73,91] 这四个数，以此类推，一共重复 18 次
            for-2 表示在 [0, 36) 这个区间中每隔 9 个选一个，也就是在 [0,8),[9,18),[18,27),[27,36) 四个区间中各选一个数，累加，然后乘以 1 / sqrt(4)
                  比如选择 [0,9,18,27] 这四个数，第二次选择 [1,10,19,28] 这四个数，以此类推，一共重复 9 次
    ...
    # 最后形成的矩阵的 shape 为 [sizeY, sizeX, 31]
    return newData
...
def getFeatureMaps(image, k, mapp):
    ...
    # 由于每一个 cell 中的像素点的幅值要按一定权重分配到相邻的 3 个 cell 中，所以需要一个矩阵来指明方位，nearest = [-1,-1,1,1]
    ...
    # mapp['map'] 是一个 shape 为 [sizeX, sizeY, p] 的矩阵，其中 p = 18 + 9 = 27，mapp['map'] 可以看成是图像中每个 cell 中向量的集合
...
def normalizeAndTruncate(mapp, alfa):
    ...
    # 这里最后计算出来的 idx 的 shape 为 (sizeX * sizeY, 9)，代表了每一个 cell 前 9 个梯度幅值的下标
    # 比如 idx 为：[[0,1,2,3,4,5,6,7,8],[27,28,29,30,31,32,33,34,35],[54,55,56,57,58,59,60,61,62],...]
    idx = np.arange(0, sizeX * sizeY * mapp['numFeatures'], mapp['numFeatures']).reshape((sizeX * sizeY, 1)) + np.arange(p)

    # partOfNorm 中的值就是上面所说的 cell 方向梯度直方图中前 9 个方向梯度的平方值的和，即一个长度为 sizeX * sizeY 的向量
    # 具体的组合运算还是得等到 func3 中去进行
    ...
    # 进行截断操作，也就是把 newData 中大于 alfa 的值设置为 alfa，alfa 的值默认为 0.2
