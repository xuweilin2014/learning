...
def _cosine_distance(a, b, data_is_normalized=False):
    ...
    if not data_is_normalized:
        ...
        #  np.linalg.norm 操作是求向量的范式，默认是L2范式
...
def _nn_cosine_distance(x, y):
    ...
    # distances.min(axis=0) 返回一个长度为 1*N 的向量，代表了这个 track 到每一个 detection 的 feature 的余弦距离的最小值
...
class NearestNeighborDistanceMetric(object):
    ...
    def distance(self, features, targets):
        ...
        # 最后计算出来的 cost_matrix[i][j] 代表 track[i] 的到 detection[j] 的 feature 余弦距离的最小值
        # 一个 detection[j] 对象一般只有一个 feature 向量
        cost_matrix = np.zeros((len(targets), len(features)))
        for i, target in enumerate(targets):
            # self.samples 存储了所有的 track 中的最近 100 个特征向量（feature），而 samples[target] 则代表 track_id 为 target 的 feature 列表
            # self._metric 方法返回的是 track_id 代表的 track 到每一个 detection 之间的最小余弦距离
