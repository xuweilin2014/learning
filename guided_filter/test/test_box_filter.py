...
class TestBoxFilter(unittest.TestCase):
    ...
    def test_box_filter_reflect_101(self):
        ...
        r = 2
        ret1 = smooth.box_filter(I, r, normalize=True)
    ...
    def test_box_filter_reflect(self):
        I = np.array(range(1, 50)).reshape(7, 7).astype(np.float32)
        r = 2
        ret1 = smooth.box_filter(I, r, normalize=True, border_type='reflect')
    ...
    def test_box_filter_edge(self):
        I = np.array(range(1, 50)).reshape(7, 7).astype(np.float32)
        r = 2
        ret1 = smooth.box_filter(I, r, normalize=True, border_type='edge')
    ...
    def test_box_filter_zero(self):
        I = np.array(range(1, 50)).reshape(7, 7).astype(np.float32)
        r = 2
        ret1 = smooth.box_filter(I, r, normalize=True, border_type='zero')
