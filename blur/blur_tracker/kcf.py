...
def fftd(img, backwards=False):
    # shape of blur_face can be (m,n), (m,n,1) or (m,n,2)
...
class KCFTracker:
    ...
    def init(self, roi, image, classifier):
        self._roi = list(map(float, roi))
        assert (self._roi[2] > 0 and self._roi[3] > 0)
        # _tmpl 提取的 fhog 特征矩阵，shape 为 [numFeatures, sizeY * sizeX]
        self._tmpl = self.get_features(self.get_extracted_img(image, 1, 1.0), 1)
    ...
    def get_extracted_img(self, image, inithann, scale_adjust=1.0):
    ...
    def get_features(self, z, inithann):
        if self._hogfeatures:
    ...
    def update(self, image):
        ...
        # if blur_feat:
        #     blur_face = z
        #     blur_face = cv2.cvtColor(blur_face, cv2.COLOR_BGR2GRAY)
        #     # blur_face = (blur_face - np.min(blur_face)) / (np.max(blur_face) - np.min(blur_face))
        #     #
        #     # f1 = local_auto_correlation(blur_face, bins=36)
        #     # f2 = gradient_histogram_span(blur_face, 11, bins=512, blur=False, display=False)
        #     # f3 = local_kurtosis(blur_face, 11, bins=512, blur=False, display=False)
        #     # f = np.concatenate((f1, f2, f3, [0]), axis=0)
        #     #
        #     # if self.clfier.predict(np.reshape(f, (1, -1))) == 1:

        # 尺度不变时检测峰值结果
        z = self.get_extracted_img(image, 0, 1.0)
        angle = poly_fit_angle(cv2.cvtColor(z, cv2.COLOR_BGR2GRAY))
        w, h, c = z.shape
        final_loc, final_peak_val, final_kernel, final_anchor = None, 0, None, None
        limit = int(np.sqrt(w ** 2 + h ** 2))
        for length in range(0, limit, 4):
            kernel, anchor = generate_blur_kernel(length, angle)
            z = cv2.filter2D(z, -1, kernel, anchor=anchor)
            loc, peak_value = self.detect(self._tmpl, self.get_features(z, 0))
            if peak_value > final_peak_val:
                final_peak_val = peak_value
                final_loc = loc
                final_kernel = kernel
                final_anchor = anchor

        norm_x = 0.05 * np.cos((np.pi / 180) * angle) * np.sign(final_loc[0])
        norm_y = 0.05 * np.sin((np.pi / 180) * angle) * np.sign(final_loc[1])
        final_loc += np.array([norm_x, norm_y])

        # 略大尺度和略小尺度进行检测，也就是使用尺度池中的 1 / 1.05 和 1.05 两个尺度因子来对原图像进行缩小和放大
        if self.scale_step != 1:
            # Test at a smaller _scale
            z1 = self.get_extracted_img(image, 0, 1.0 / self.scale_step)
            z1 = cv2.filter2D(z1, -1, final_kernel, anchor=final_anchor)
            new_loc1, new_peak_value1 = self.detect(self._tmpl, self.get_features(z1, 0))

            # Test at a bigger _scale
            z2 = self.get_extracted_img(image, 0, self.scale_step)
            z2 = cv2.filter2D(z2, -1, final_kernel, anchor=final_anchor)
            new_loc2, new_peak_value2 = self.detect(self._tmpl, self.get_features(z2, 0))

            # 对于不同的尺度，都有着尺度惩罚系数 scale_weight，用此系数乘以该尺度下的最大响应值作为该尺度下的真实最大响应值
            # 取最大响应值对应的尺度为最佳尺度，也就是 self._scale
            if self.scale_weight * new_peak_value1 > final_peak_val and new_peak_value1 > new_peak_value2:
                loc = new_loc1
                peak_value = new_peak_value1
                self._scale /= self.scale_step
                self._roi[2] /= self.scale_step
                self._roi[3] /= self.scale_step
            elif self.scale_weight * new_peak_value2 > final_peak_val:
                loc = new_loc2
        ...
        self._roi[0] = cx - self._roi[2] / 2.0 + final_loc[0] * self.cell_size * self._scale
        self._roi[1] = cy - self._roi[3] / 2.0 + final_loc[1] * self.cell_size * self._scale
        ...
        x = self.get_features(self.get_extracted_img(image, 0, 1.0), 0)
        self.train(x, self.interp_factor)

        return self._roi, final_peak_val
