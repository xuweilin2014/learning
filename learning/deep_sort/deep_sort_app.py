...
3.调用 Tracker#predict 方法，依据已经得到的 track，对当前帧进行跟踪结果进行预测，预测得到跟踪物体在下一帧中的位置，预测完之后，需要对每一个 tracker 的 time_since_update 加一
4.调用 Tracker#update 方法，对 track 进行更新，更新的具体步骤如下：
    ...
    循环遍历到最多已经有 max_age 次没有匹配到的 track（time_since_update = max_age 的 track），这样做的目的是为了对更加频繁出现的目标赋予优先权，级联匹配的具体步骤如下：
        ...
        最小值作为该 track 与检测结果 detection 之间的计算值，比如对于一个 track 中 N 个 feature 集和 M 个 detection 的 feature，有如下 cost 矩阵
        （df 表示 detection 的 feature，而 tf 表示 track 中的 feature 特征集）：
        ...
        最后会变成一个 1*M 的矩阵，表示的是 dfi 到 track 的所有 feature 之间的余弦距离的最小值
    ...
    3).unconfirmed tracks 和 2) 中没有匹配的 tracker（unmatched_tracks_a）一起组成 iou_track_candidates，与还没有匹配上的检测结果 unmatched_detections 进行
