...
def matching_cascade(distance_metric, max_distance, cascade_depth, tracks, detections, track_indices=None, detection_indices=None):
    ...
    # 不过在这里，track_indices 中的 index 是 confirmed track 的 index
    # detection_indices 为 null，所以需要创建一个 detection_indices，表示 detections 集合的索引
    if track_indices is None:
    ...
    for level in range(cascade_depth):
        ...
        # detection 只和 time_since_update == level + 1 的 track 进行匹配
        track_indices_l = [
