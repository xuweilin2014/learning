...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=30, n_init=3):
        ...
        # 最大 iou 阈值，iou 匹配的时候使用
        self.max_iou_distance = max_iou_distance

        # max_age 直接指定级联匹配的 cascade_depth 参数的值，如果一个处于 confirmed 状态的 track，最多连续 max_age 都没有匹配上任何 detection，
        # 那么就会被删除
    ...
    def update(self, detections):
        ...
        # 下面将每一个 confirmed 状态的 track 的 feature 向量（或者向量集合）保存到 samples 字典中
        # 并且在 samples 字典中，每一个 track 最多只保存最近 budget 个 feature 向量
        ...
        for track in self.tracks:
            ...
            # 生成长度为 len(track.features) 的 targets 对象，其中保存的都是 track_id，
            # 并且其中的 track_id 和上面的 feature 所属于的 track 是一一对应的
        ...
        # 实际上 track 中的 features 只暂时存储特征向量，features 最终都是保存在 simples 字典中
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            # 每一个 track 到每一个 detection 之间的距离是这样计算的，因为一个 track 有多个 feature，所以会计算多个 feature 到这个 detection
            # 之间的距离，取最小值作为此 track 到 detection 之间距离值
    ...
    def _initiate_track(self, detection):
        ...
        # 初始化为 [center_x, center_y, w/h, h, 0, 0, 0, 0] 与协方差矩阵（8*8 矩阵）
